...
# set :yarn_target_path, -> { current_path.join('bin') } #=> なぜ /usr/bin/env になるのだ……
# set :yarn_target_path, -> { "/home/deploy_gss_2018_development/deploy/suikoden-election-2018/current/bin" }
# set :yarn_target_path, -> { release_path.join('bin') } #=> なぜ /usr/bin/env になるのだ……
# "#{release_path}/config/unicorn.development.rb"
set :yarn_target_path, -> { "#{release_path}/bin/yarn" } #=> なぜ /usr/bin/env になるのだ……
