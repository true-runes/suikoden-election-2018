...
class Tweet < ApplicationRecord
  ...
  scope :valid_vote_tweets_with_order_by, lambda { |order_by: nil|
                                            Tweet.without_deleted.where(is_retweet_validation).where(tweeted_at_validation).where.not(user_id_validation).order(tweeted_at: order_by)
                                          }
  ...
  scope :now_counting_tweets_desc, lambda {
                                     Tweet.without_deleted.where(is_retweet_validation).where(tweeted_at_validation_for_now_counting).where(user_id_validation).order(tweeted_at: :desc)
                                   }
  ...
  scope :latest_original_tweet_of_specific_user_id, lambda { |user_id|
                                                      Tweet.without_deleted.where(is_retweet: false).where(user_id:).order(tweet_number: :desc).first
                                                    }
  ...
  def latest_original_tweet_number_of_specific_screen_name(screen_name)
    unless User.where(screen_name:).empty?
      user_id = User.where(screen_name:).first.id

      if Tweet.without_deleted.where(is_retweet: false).where(user_id:).empty?
        1
      else
        Tweet.without_deleted.where(is_retweet: false).where(user_id:).order(tweet_number: :desc).first.tweet_number
      end
    end
  end
  ...
  def check_duplicate_vote_users
    ...
    users_with_valid_vote.each do |user|
      @duplicate_vote_users << user if User.new.user_with_valid_votes_count(user) >= 2
    end
