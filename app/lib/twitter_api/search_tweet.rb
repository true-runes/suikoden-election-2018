...
module TwitterApi
  class SearchTweet
    ...
    def self.latest_tweet_number(search_word:)
      search_word_tweet_exists?(search_word:) ? SearchWord.where(word: search_word).first.tweets.order(tweet_number: :desc).first[:tweet_number].to_s : 0
    end

    def self.oldest_tweet_number(search_word:)
      search_word_tweet_exists?(search_word:) ? SearchWord.where(word: search_word).first.tweets.order(tweet_number: :asc).first[:tweet_number].to_s : 0
    end
  end
end
